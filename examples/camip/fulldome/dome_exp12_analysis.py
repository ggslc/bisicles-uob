...
def r_analytic(time):
    dr =  np.where(time > 0, 300.0/(2*np.pi) * (np.cos(time*2.0*np.pi/1000) - 1.0),0)
    return 750.0 + dr
...
def plot_state(bike, ax):
    ...
    ax.plot(*circ(r_analytic(bike.time-100)), color='magenta', ls='--', lw=2, label='analytic front')
    #ax.set_xlabel('x (km)')
    ax.set_ylabel('y (km)')
    
    dx = km(bike.x[1] - bike.x[0])
    ax.set_title(f't = {bike.time-100} a, dx_min = {dx} km')
...
def plot_ts(filespec, level):
    ...
    mid = BisiclesData(files[int(n/4)], level=level,iord=0)
    plot_state(mid, axm)
...
t0, a0, b0 = plot_ts('plot.camip_fulldome_expt2_tc-1.??????.2d.hdf5', level=0)    
#t1, a1, b1 = plot_ts('plot.camip_fulldome_expt2_tc0.??????.2d.hdf5', level=1)   
#t2, a2, b2 = plot_ts('plot.camip_fulldome_expt2_tc1.??????.2d.hdf5', level=2)
...
plt.plot(t0, ff(radius(a0) - r_analytic(t0)),label='5 km ')
#plt.plot(t1, ff(radius(a1) - r_analytic(t1)),label='2.5 km')
#plt.plot(t2, ff(radius(a2) - r_analytic(t2)),label='1.25 km')
#plt.plot(t3, ff(radius(a3) - r_analytic(t3)),label='0.625 km')
#plt.plot(t4, ff(radius(a4) - r_analytic(t4)),label='0.3125 km')
#plt.plot(t0, radius(c0) - r_analytic(t0),label='0')
#plt.plot(t1, radius(c1) - r_analytic(t1),label='1')
#plt.plot(t2, radius(c2) - r_analytic(t2),label='2')



plt.xlabel(r'time, $t$ (a)')
plt.ylabel(r'Error: $|A/200 - r(t)|$ (km)')
loge = np.arange(-4,2.0,1)
#plt.yticks(loge, 2.0**loge)
plt.legend(title='dx')
#plt.yticks(np.arange(0,10,1.25))
plt.grid()
