...
def areas(bike):
    ...
    ag = np.sum(np.where(bike.hab > 1, 1.0, 0.0))*dx**2
...
t0, a0, b0 = plot_ts('plot.camip_fullthule_spin_tc-1.??????.2d.hdf5', level=0)    
t1, a1, b1 = plot_ts('plot.camip_fullthule_spin_tc0.??????.2d.hdf5', level=1)   
t2, a2, b2 = plot_ts('plot.camip_fullthule_spin_tc1.??????.2d.hdf5', level=2)   
t3, a3, b3 = plot_ts('plot.camip_fullthule_spin_tc2.??????.2d.hdf5', level=3)  
#t4, a4, b4 = plot_ts('plot.camip_fullthule_spin_tc3.??????.2d.hdf5', level=4) 
# %%
fig = plt.figure(figsize=(4,4), dpi=200)
#plt.plot(ot0,ob0**0.5,'o--',label='0 (old)')
#plt.plot(ot1,ob1**0.5,'o--',label='1 (old)')
plt.plot(t0,b0**0.5,'s-',label='0',ms=2)
plt.plot(t1,b1**0.5,'s-',label='1',ms=2)
plt.plot(t2,b2**0.5,'s-',label='2',ms=2)
plt.plot(t3,b3**0.5,'s-',label='3',ms=2)
...
plt.xlabel('time')
plt.ylabel(' sqrt(grounded area) (km)^2')
plt.grid()
