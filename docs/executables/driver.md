...
The BISICLES `driver` executable is used to run standalone time-dependent ice sheet simulations.

## Startup Operations

At the start of a simulation, the driver carries out the following operations:

1. Read a [run-time configuration](../configuration/io.md#run-time-configuration) file, specified as the first command line argument

2. Compute initial thickness, topography, internal energy, stiffness and basal friction coefficient data over the whole domain on the base level (l = 0) mesh. This data might be loaded from a file through the [LevelData interface](../configuration/leveldatainterface.md), computed through the [Python interface](../configuration/pythoninterface.md) or through a number of special-purpose modules.
...
8. If L has not reached its maximum value, set L = L + 1 and repeat steps 3-7

## Main Simulation Loop

After the startup phase, the driver runs the following operations repeatedly until the simulation has completed:
...
It is also possible to [restart](../configuration/io.md#checkpoints-and-restarts) a simulation from a checkpoint file, for example to create a number of simulation branches, having carried out some sort of relaxation.
